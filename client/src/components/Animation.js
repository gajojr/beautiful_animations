...
async function getLikedAnimations(username) {
  const res = await fetch(
    `http://localhost:8080/animation-list/?username=${username}`
  );
...
const Animation = ({ name, gif, description, link }) => {
  ...
  if (!sessionStorage.getItem("username")) {
    getUsernameAndAnimations(
      "http://localhost:8080/login/send-data-to-frontend"
    ).then((data) => {
    ...
    <div className="block">
      <div className="naslov-link">
        ...
        <span className="animation-link-header">
          <a href={link || "https://www.google.rs/"} className="animation-link">
            Visit
          </a>
        </span>
      </div>
          ...
          <IconContext.Provider value={{ color: "red", size: 34 }}>
            <div
              ...
              onClick={() => {
                ...
                getLikedAnimations(sessionStorage.getItem("username")).then(
                  (animations) => {
            ...
            >
              {likedAnimations.indexOf(link) !== -1 ? (
                <AiFillHeart />
              ) : (
                <AiOutlineHeart />
              )}
